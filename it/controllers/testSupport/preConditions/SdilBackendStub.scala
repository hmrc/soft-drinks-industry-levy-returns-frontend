...
import models.{FinancialLineItem, LevyCalculation, ReturnCharge, ReturnPeriod}
import play.api.libs.json.Json

import java.time.LocalDate

case class SdilBackendStub()(implicit builder: PreconditionBuilder) {
  ...
  val aSubscriptionWithDeRegDate: RetrievedSubscription = aReceivedSubscription.copy(deregDate = Some(LocalDate.of(2022, 2, 11)))
  ...
  def retrieveSubscriptionToModify(identifier: String, refNum: String, retrievedSubscription: RetrievedSubscription): PreconditionBuilder = {
    stubFor(
      get(urlPathMatching(s"/subscription/$identifier/$refNum"))
        .willReturn(ok(Json.toJson(retrievedSubscription).toString()))
    )
    builder
  }
  ...
  def retrieveSubscriptionWithDeRegDate(identifier: String, refNum: String): PreconditionBuilder = {
    stubFor(
      get(urlEqualTo(s"/subscription/$identifier/$refNum"))
        .willReturn(ok(Json.toJson(aSubscriptionWithDeRegDate).toString()))
    )
    builder
  }
  ...
  def retrieveSubscriptionNone(identifier: String, refNum: String): PreconditionBuilder = {
    stubFor(
      get(urlPathEqualTo(s"/subscription/$identifier/$refNum"))
        .willReturn(notFound())
    )
    builder
  }
  ...
  def pendingReturnPeriod(utr: String): PreconditionBuilder = {
    stubFor(
      get(urlPathMatching(s"/returns/$utr/pending"))
        .willReturn(ok(Json.toJson(List(returnPeriod)).toString()))
    )
    builder
  }
  ...
  def pendingReturnPeriodNone(utr: String): PreconditionBuilder = {
    stubFor(
      get(urlPathMatching(s"/returns/$utr/pending"))
        .willReturn(ok(Json.toJson(List.empty[ReturnPeriod]).toString()))
    )
    builder
  }
  ...
  def balance(sdilRef: String, withAssessment: Boolean): PreconditionBuilder = {
    stubFor(
      get(urlPathMatching(s"/balance/$sdilRef/$withAssessment"))
        .willReturn(ok(Json.toJson(BigDecimal(10000)).toString()))
    )
    builder
  }
  ...
  def balanceNegative(sdilRef: String, withAssessment: Boolean): PreconditionBuilder = {
    stubFor(
      get(urlPathMatching(s"/balance/$sdilRef/$withAssessment"))
        .willReturn(ok(Json.toJson(BigDecimal(-1000)).toString()))
    )
    builder
  }
  ...
  def balanceNone(sdilRef: String, withAssessment: Boolean): PreconditionBuilder = {
    stubFor(
      get(urlPathMatching(s"/balance/$sdilRef/$withAssessment"))
        .willReturn(ok(Json.toJson(BigDecimal(0)).toString()))
    )
    builder
  }
  def submitReturns(utr: String = "0000001611", year: Int = 2018, quarter: Int = 1): PreconditionBuilder = {
    stubFor(
      post(urlPathMatching(s"/returns/$utr/year/$year/quarter/$quarter"))
        .willReturn(ok())
    )
    builder
  }
  ...
  def balanceHistoryNone(sdilRef: String): PreconditionBuilder = {
    stubFor(
      get(urlPathMatching(s"/balance/$sdilRef/history/all/false"))
        .willReturn(ok(Json.toJson[Seq[FinancialLineItem]](List(ReturnCharge(returnPeriod, BigDecimal(0)))).toString()))
    )
    builder
  }
  ...
  def balanceHistoryInCredit(sdilRef: String): PreconditionBuilder = {
    stubFor(
      get(urlPathMatching(s"/balance/$sdilRef/history/all/false"))
        .willReturn(ok(Json.toJson[Seq[FinancialLineItem]](List(ReturnCharge(returnPeriod, BigDecimal(-1000)))).toString()))
    )
    builder
  }
    ...
    stubFor(
      get(urlPathMatching(s"/balance/$sdilRef/history/all/false"))
        .willReturn(
          ok(
            Json
              .toJson[Seq[FinancialLineItem]](
                List(ReturnCharge(returnPeriod, BigDecimal(1000)), ReturnCharge(ReturnPeriod(2018, 2), BigDecimal(1000)))
              )
              .toString()
          )
        )
  ...
  def calculateLevy(lowBandLevy: BigDecimal, highBandLevy: BigDecimal, lowLitres: Long, highLitres: Long): PreconditionBuilder = {
  ...
  def calculateLevyDefault(): PreconditionBuilder = {
