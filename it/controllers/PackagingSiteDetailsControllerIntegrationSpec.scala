  ...
  "PackagingSiteDetailsController" should {
    ...
    "Post the request to update packaging site details " when {
      ...
      "user selected yes, user should be taken to ALF" in {
        val journeyConfigToBePosted: JourneyConfig = JourneyConfig(
          ...
          options = JourneyOptions(
            ...
            phaseFeedbackLink = Some(
              s"http://localhost:9250/contact/beta-feedback?service=soft-drinks-industry-levy-returns-frontend&backUrl=http%3A%2F%2Flocalhost%3A8703%2Fsoft-drinks-industry-levy-returns-frontend%2Fpackaging-site-details"
            ),
        ...
        setUpData(
          UserAnswers(sdilNumber, requestReturnPeriod, Json.obj("HowManyBroughtIntoUk" -> Json.obj("lowBand" -> 10, "highBand" -> 10)), List.empty)
        )
        build
        ...
        WsTestClient.withClient { client =>
          val result =
            client
              .url(s"$baseUrl/packaging-site-details")
              .addCookies(DefaultWSCookie("mdtp", authAndSessionCookie))
              .withHttpHeaders("X-Session-ID" -> "XCSDIL000000069", "Csrf-Token" -> "nocheck")
    ...
    "Post the request to continue from packaging site details " when {
      ...
      "user selected no with at least one packaging site on the list" in {
        ...
        val expectedResult: Some[Map[String, Site]] = Some(
          Map(
            "4564561" -> Site(
        ...
        WsTestClient.withClient { client =>
          val result =
            client
              .url(s"$baseUrl/packaging-site-details")
              .addCookies(DefaultWSCookie("mdtp", authAndSessionCookie))
              .withHttpHeaders("X-Session-ID" -> "XSSDIL000000232", "Csrf-Token" -> "nocheck")
              .withFollowRedirects(false)
              .post(Json.obj("value" -> "false"))
