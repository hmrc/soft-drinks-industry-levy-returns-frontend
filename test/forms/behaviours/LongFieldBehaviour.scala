...
class LongFieldBehaviour extends FieldBehaviours {
  def longField(form: Form[?], fieldName: String, nonNumericError: FormError, negativeNumberError: FormError, wholeNumberError: FormError): Unit = {
  ...
  def longFieldWithMaximum(form: Form[?], fieldName: String, maximum: Long, expectedError: FormError): Unit = {

    s"not bind long above $maximum" in
      forAll(longAboveValue(maximum) -> "outOfMaxVal") { (number: Long) =>
