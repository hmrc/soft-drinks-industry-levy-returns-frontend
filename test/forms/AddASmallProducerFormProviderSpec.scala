...
class AddASmallProducerFormProviderSpec
    extends LongFieldBehaviour
    with StringFieldBehaviours
    with SDILReferenceFieldBehaviours
    with SpecBase
    with MockitoSugar {
  ...
  ".referenceNumber" - {
    ...
    behave like invalidSDILFormat(form, fieldName, requiredError = FormError(fieldName, invalidSDILFormatKey))

    behave like invalidSDILRef(form, fieldName, requiredError = FormError(fieldName, invalidSDILRefKey))
