...
class LevyCalculatorSpec extends SpecBase with ScalaCheckPropertyChecks {
  ...
  "getTaxYear" - {
    ...
    "return numeric value of year - 1 when in January" in {
      forAll(yearGen) { year =>
        ...
        getTaxYear(returnPeriod) mustBe year - 1
      }
    }
  ...
  "getBandRates" - {
    (2018 to 2024).foreach(taxYear => {
      val bandRates: BandRates = getBandRates(taxYear)(frontendAppConfig)
    ...
    "return 0.194 for lower band when tax year is 2025" in {
      val bandRates: BandRates = getBandRates(2025)(frontendAppConfig)
