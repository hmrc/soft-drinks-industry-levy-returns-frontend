...
class RichJsValueSpec extends AnyFreeSpec with Matchers with ScalaCheckPropertyChecks with OptionValues with ModelGenerators {
  ...
  "set" - {
    ...
    "must set into an array which does not exist" in {
      ...
      value.set(path, JsString("bar")) mustEqual JsSuccess(Json.obj("foo" -> Json.arr("bar")))
    }
    ...
    "must set into an object which does not exist" in {
      ...
      value.set(path, JsString("baz")) mustEqual JsSuccess(Json.obj("foo" -> Json.obj("bar" -> "baz")))
    }
    ...
    "must set nested objects and arrays" in {
      ...
      value.set(path, JsString("baz")) mustEqual JsSuccess(Json.obj("foo" -> Json.arr(Json.obj("bar" -> Json.arr("baz")))))
    }
  }
  ...
  "remove" - {
    ...
    "must remove a value given an index node and return the new object for one array" in {
      ...
      forAll(gen) { case (key: String, values: List[String], indexToRemove: Int) =>
        ...
        val expectedOutcome =
          buildJsObj(Seq(key), Seq(valuesInArrays.slice(0, indexToRemove) ++ valuesInArrays.slice(indexToRemove + 1, values.length)))
    ...
    "remove a value from one of many arrays" in {
      ...
      val input = Json.obj("key" -> JsArray(Seq(Json.toJson(1), Json.toJson(2))), "key2" -> JsArray(Seq(Json.toJson(1), Json.toJson(2))))

      val path = JsPath \ "key" \ 0

      input.remove(path) mustBe JsSuccess(Json.obj("key" -> JsArray(Seq(Json.toJson(2))), "key2" -> JsArray(Seq(Json.toJson(1), Json.toJson(2)))))
    }
  }
  ...
  "remove a value when there are nested arrays" in {
    ...
    input.remove(path) mustBe JsSuccess(
      Json.obj("key" -> JsArray(Seq(JsArray(Seq(Json.toJson(2))), Json.toJson(2))), "key2" -> JsArray(Seq(Json.toJson(1), Json.toJson(2))))
    )
  }
  ...
  "remove the value if the last value is deleted from an array" in {
    ...
    input.remove(path) mustBe JsSuccess(Json.obj("key" -> JsArray(), "key2" -> JsArray(Seq(Json.toJson(1), Json.toJson(2)))))
  }
}
