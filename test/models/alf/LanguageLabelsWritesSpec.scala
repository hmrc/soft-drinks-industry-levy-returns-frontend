...
class LanguageLabelsWritesSpec extends AnyWordSpec with Matchers {
  ...
  "LanguageLabels JSON writes" should {
    ...
    "serialize a single nested section and omit its None fields" in {
      val labels = LanguageLabels(appLevelLabels = Some(AppLevelLabels(navTitle = Some("Service name"), phaseBannerHtml = None)))
    ...
    "serialize SelectPageLabels with a subset of fields" in {
      val labels = LanguageLabels(
        selectPageLabels =
          Some(SelectPageLabels(title = Some("Choose an address"), heading = Some("Select an address"), submitLabel = Some("Continue")))
      )
    ...
    "serialize a rich nested structure and omit None fields across the tree" in {
      val labels = LanguageLabels(
        appLevelLabels = Some(AppLevelLabels(navTitle = Some("My Service"), phaseBannerHtml = Some("Beta – give feedback"))),
        ...
        countryPickerLabels = Some(
          CountryPickerPageLabels(
