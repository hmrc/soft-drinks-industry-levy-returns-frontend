...
class AddressLookupServiceSpec extends SpecBase with FutureAwaits with DefaultAwaitTimeout {
  ...
  "getAddress" - {
    "return an address when Connector returns success" in {
      when(mockSdilConnector.getAddress("123456789")(using hc, implicitly)).thenReturn(Future.successful(Right(customerAddressMax)))
    ...
    "return an exception when Connector returns error" in {
      when(mockSdilConnector.getAddress("123456789")(using hc, implicitly)).thenReturn(Future.successful(Left(ErrorModel(1, "foo"))))
  ...
  "addAddressUserAnswers" - {
    ...
    s"add to the cache the address of a $WarehouseDetails when a user returns from address lookup frontend with missing address lines" in {
      ...
      val customerAddressMissingLines: AlfAddress =
        AlfAddress(Some(organisation), List(addressLine1, addressLine2), Some(postcode), Some(countryCode))
    ...
    s"add to the cache the address of a $WarehouseDetails when a user returns from address lookup frontend with full address lines" in {
      ...
      val res = service.addAddressUserAnswers(
        addressLookupState = addressLookupState,
        address = AlfAddress(Some(organisation), List(addressLine1, addressLine2, addressLine3, addressLine4), Some(postcode), Some(countryCode)),
        userAnswers = emptyUserAnswers.copy(warehouseList = warehouseMap),
        alfId = alfId,
        siteId = sdilId
      )
    ...
    s"add to the cache the address of a $PackingDetails when a user returns from address lookup frontend with full address lines and sdilRef DOESN'T exist" in {
      ...
      val customerAddressMissingLines: AlfAddress =
        AlfAddress(Some(organisation), List(addressLine1, addressLine2, addressLine3, addressLine4), Some(postcode), Some(countryCode))
    ...
    "don't add to userAnswers when no details are added in alf and throw exception" in {
      val addressLookupState = WarehouseDetails
      val warehouseMap       = Map("1" -> Site(UkAddress(List("33 Rhes Priordy", "East London"), "E73 2RP"), None, Some("super cola"), None))
      val customerAddressMissingLinesAndName: AlfAddress = AlfAddress(None, List(), None, None)
  ...
  "initJourney" - {
    "should return response from connector" in {
      ...
      when(mockSdilConnector.initJourney(ArgumentMatchers.eq(journeyConfig))(using ArgumentMatchers.any(), ArgumentMatchers.any()))
  ...
  "initJourneyAndReturnOnRampUrl" - {
    s"should return Successful future when connector returns success for $PackingDetails" in {
      ...
      val expectedJourneyConfigToBePassedToConnector = JourneyConfig(
        ...
        options = JourneyOptions(
          ...
          phaseFeedbackLink = Some(
            s"http://localhost:9250/contact/beta-feedback?service=soft-drinks-industry-levy-returns-frontend&backUrl=http%3A%2F%2Flocalhost%3A8703bar"
          ),
          ...
          ukMode = Some(true),
          selectPageConfig = Some(SelectPageConfig(proposalListLimit = Some(10), showSearchAgainLink = Some(true))),
        ...
        labels = Some(
          JourneyLabels(
            en = Some(
              LanguageLabels(
                appLevelLabels = Some(AppLevelLabels(navTitle = Some("Soft Drinks Industry Levy"), phaseBannerHtml = None)),
      ...
      when(
        mockSdilConnector.initJourney(ArgumentMatchers.eq(expectedJourneyConfigToBePassedToConnector))(using
          ArgumentMatchers.any(),
          ArgumentMatchers.any()
        )
      )
        .thenReturn(Future.successful(Right("foo")))
      whenReady(service.initJourneyAndReturnOnRampUrl(PackingDetails, sdilId)(using implicitly, implicitly, implicitly, FakeRequest("foo", "bar"))) {
        res => res mustBe "foo"
      }
    }
    s"should return Successful future when connector returns success for $WarehouseDetails" in {
      ...
      when(
        ...
        .thenReturn(Future.successful(Right("foo")))
      whenReady(service.initJourneyAndReturnOnRampUrl(WarehouseDetails, sdilId)(using implicitly, implicitly, implicitly, FakeRequest("foo", "bar"))) {
        res => res mustBe "foo"
      }
    }
    ...
    "should return Exception if connector returns left" in {
      ...
      val res = intercept[Exception](
        await(service.initJourneyAndReturnOnRampUrl(PackingDetails)(using implicitly, implicitly, implicitly, FakeRequest("foo", "bar")))
      )
  ...
  "createJourneyConfig" - {
    s"should return a journey config for $WarehouseDetails" in {
      ...
      val res      = service.createJourneyConfig(WarehouseDetails, exampleSdilIdWeGenerate, NormalMode)(using request, implicitly)
      val expected = JourneyConfig(
        ...
        options = JourneyOptions(
          ...
          accessibilityFooterUrl = Some("localhost/accessibility-statement/soft-drinks-industry-levy-returns-frontend"),
          phaseFeedbackLink = Some(
            s"http://localhost:9250/contact/beta-feedback?service=soft-drinks-industry-levy-returns-frontend&backUrl=http%3A%2F%2Flocalhost%3A8703${request.uri}"
          ),
          deskProServiceName = None,
          showPhaseBanner = Some(false),
          alphaPhase = Some(frontendAppConfig.AddressLookupConfig.alphaPhase),
    ...
    s"should return a journey config for $PackingDetails" in {
      val request = FakeRequest("foo", "bar")
      val exampleSdilIdWeGenerate: String = "wizz"
      val res      = service.createJourneyConfig(PackingDetails, exampleSdilIdWeGenerate)(using request, implicitly)
      val expected = JourneyConfig(
        version = 2,
        options = JourneyOptions(
