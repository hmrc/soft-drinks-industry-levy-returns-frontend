...
class AddressLookupHttpParserSpec extends SpecBase {
  ...
  "The AddressLookupGetAddressReads" - {
    ...
    "the http response status is OK with valid Json" - {
      ...
      "return a AddressLookupModel" in {
        AddressLookupGetAddressReads.read("", "", HttpResponse(Status.OK, customerAddressMaxJson, Map.empty[String, Seq[String]])) mustBe Right(
          customerAddressMax
        )
      }
    }
    ...
    "the http response status is OK with invalid Json" - {
      ...
      "return an ErrorModel" in {
        AddressLookupGetAddressReads.read("", "", HttpResponse(Status.OK, customerAddressJsonError, Map.empty[String, Seq[String]])) mustBe
  ...
  "The AddressLookupInitJourneyReads" - {
    s"should return url if ${Status.ACCEPTED} returned and ${HeaderNames.LOCATION} exists" in {
      AddressLookupInitJourneyReads.read("", "", HttpResponse(Status.ACCEPTED, "", Map(HeaderNames.LOCATION -> Seq("foo")))) mustBe Right("foo")
    }
