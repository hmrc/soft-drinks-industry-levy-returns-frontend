...
class RequiredUserAnswersSpec extends SpecBase {
  ...
  val basicRequestWithEmptyAnswers = DataRequest(
    FakeRequest(),
    "",
    RetrievedSubscription(
      ...
      RetrievedActivity(true, true, true, true, true),
      LocalDate.now(),
      List.empty,
      List.empty,
      Contact(None, None, "", ""),
  ...
  val basicJourney: List[RequiredPage[?, ?, ?]] = List(
    ...
    RequiredPage(HowManyBroughtIntoUkPage, Some(PreviousPage(BroughtIntoUKPage, true)(implicitly[Reads[Boolean]])))(implicitly[Reads[LitresInBands]]),
  ...
  val smallProducerFalseJourney: List[RequiredPage[?, ?, ?]] = List(
    RequiredPage(OwnBrandsPage, None)(implicitly[Reads[Boolean]]),
    RequiredPage(BrandsPackagedAtOwnSitesPage, Some(PreviousPage(OwnBrandsPage, true)(implicitly[Reads[Boolean]])))(implicitly[Reads[LitresInBands]])
  )

  val coPackerFalseJourney: Seq[RequiredPage[QuestionPage[Boolean], PreviousPage[?, ?], Boolean]] = List(
  ...
  "checkYourAnswersRequiredData" - {
    ...
    "should return Redirect to start page when user answers is empty and the small producer is true" in {
      val res = requiredUserAnswers.checkYourAnswersRequiredData(Future.successful(Ok("")))(using basicRequestWithEmptyAnswers)
    ...
    "should return Redirect to start page when user answers is empty and the small producer is false" in {
      val subscription = RetrievedSubscription(
        ...
        "",
        UkAddress(List.empty, "", None),
        RetrievedActivity(false, true, true, true, true),
        LocalDate.now(),
        List.empty,
        List.empty,
        Contact(None, None, "", ""),
      ...
      val res = requiredUserAnswers.checkYourAnswersRequiredData(Future.successful(Ok("")))(using
        basicRequestWithEmptyAnswers.copy(subscription = subscription)
      )
  ...
  "requireData" - {
    "should take a random page and allow user to carry on their action with empty answers" in {
      val res = requiredUserAnswers.requireData(OwnBrandsPage)(Future.successful(Ok("")))(using basicRequestWithEmptyAnswers)
      status(res) mustBe OK
    }
    s"should check for $CheckYourAnswersPage and redirect to start page when answers incomplete but not a nil return and small producer is false" in {
      ...
      val res = requiredUserAnswers.requireData(CheckYourAnswersPage)(Future.successful(Ok("")))(using
        basicRequestWithEmptyAnswers.copy(subscription = subscription, userAnswers = emptyUserAnswers.copy(data = Json.obj("foo" -> "bar")))
      )
      redirectLocation(res).get mustBe controllers.routes.OwnBrandsController.onPageLoad(NormalMode).url
    }
    ...
    s"should check for $CheckYourAnswersPage and redirect to start page when answers incomplete but not a nil return and small producer is true" in {
      val res = requiredUserAnswers.requireData(CheckYourAnswersPage)(Future.successful(Ok("")))(using
        basicRequestWithEmptyAnswers.copy(userAnswers = emptyUserAnswers.copy(data = Json.obj("foo" -> "bar")))
      )
      redirectLocation(res).get mustBe controllers.routes.PackagedContractPackerController.onPageLoad(NormalMode).url
    }
    s"should check for $CheckYourAnswersPage and redirect to start page when answers data is empty" in {
      val res = requiredUserAnswers.requireData(CheckYourAnswersPage)(Future.successful(Ok("foo")))(using basicRequestWithEmptyAnswers)
      redirectLocation(res).get mustBe controllers.routes.PackagedContractPackerController.onPageLoad(NormalMode).url
    }
  ...
  "returnMissingAnswers" - {
    "should return all missing answers in a list when user answers is empty" in {
      implicit val req: DataRequest[AnyContentAsEmpty.type] = basicRequestWithEmptyAnswers
    ...
    "should return SOME missing answers when SOME answers are populated" in {
      ...
      implicit val req: DataRequest[AnyContentAsEmpty.type] = basicRequestWithEmptyAnswers.copy(userAnswers = someAnswersCompleted)
      val res = requiredUserAnswers.returnMissingAnswers(requiredUserAnswers.mainRoute)
      res mustBe List(
        RequiredPage(PackagedContractPackerPage, None)(implicitly[Reads[Boolean]]),
        RequiredPage(ClaimCreditsForExportsPage, None)(implicitly[Reads[Boolean]])
  ...
  "mainRoute" - {
    ...
    "should return all correct answers if user is newImporter && newCopacker && small producer true" in {
      ...
      val subscription = RetrievedSubscription(
        ...
        "",
        UkAddress(List.empty, "", None),
        RetrievedActivity(smallProducer = true, false, contractPacker = false, importer = false, false),
        LocalDate.now(),
        List.empty,
        List.empty,
        Contact(None, None, "", ""),
      ...
      val res =
        requiredUserAnswers.mainRoute(using basicRequestWithEmptyAnswers.copy(userAnswers = completedUserAnswers, subscription = subscription))
    ...
    "should return all correct answers if user is newImporter && NOT newCopacker && small producer true" in {
      ...
      val subscription = RetrievedSubscription(
        ...
        "",
        UkAddress(List.empty, "", None),
        RetrievedActivity(smallProducer = true, true, contractPacker = true, importer = false, true),
        LocalDate.now(),
        List.empty,
        List.empty,
        Contact(None, None, "", ""),
    ...
    "should return all correct answers if user is NOT newImporter && newCopacker && small producer true" in {
      ...
      val subscription = RetrievedSubscription(
        ...
        "",
        UkAddress(List.empty, "", None),
        RetrievedActivity(smallProducer = true, true, contractPacker = false, importer = true, true),
        LocalDate.now(),
        List.empty,
        List.empty,
        Contact(None, None, "", ""),
    ...
    "should return all correct answers if user is NOT newImporter && NOT newcopacker &&  small producer true" in {
      ...
      val subscription = RetrievedSubscription(
        ...
        "",
        UkAddress(List.empty, "", None),
        RetrievedActivity(smallProducer = true, true, contractPacker = true, importer = true, true),
        LocalDate.now(),
        List.empty,
        List.empty,
        Contact(None, None, "", ""),
    ...
    "should return all correct answers if user is newImporter && newcopacker && small producer false" in {
      ...
      val subscription = RetrievedSubscription(
        ...
        "",
        UkAddress(List.empty, "", None),
        RetrievedActivity(smallProducer = false, true, contractPacker = false, importer = false, true),
        LocalDate.now(),
        List.empty,
        List.empty,
        Contact(None, None, "", ""),
    ...
    "should return all correct answers if user is NOT newImporter && NOT new co packer && NOT small producer" in {
      ...
      val subscription = RetrievedSubscription(
        ...
        "",
        UkAddress(List.empty, "", None),
        RetrievedActivity(smallProducer = false, true, contractPacker = true, importer = true, true),
        LocalDate.now(),
        List.empty,
        List.empty,
        Contact(None, None, "", ""),
  ...
  "packingListReturnChange" - {
    "should be correct if user is NOT a new packer" in {
      val subscription = RetrievedSubscription(
        ...
        "",
        UkAddress(List.empty, "", None),
        RetrievedActivity(true, true, contractPacker = true, true, true),
        LocalDate.now(),
        List.empty,
        List.empty,
        Contact(None, None, "", ""),
    ...
    "should be correct if user is a new packer" in {
      ...
      val subscription = RetrievedSubscription(
        ...
        "",
        UkAddress(List.empty, "", None),
        RetrievedActivity(true, true, contractPacker = false, true, true),
        LocalDate.now(),
        List.empty,
        List.empty,
        Contact(None, None, "", ""),
      ...
      val res = requiredUserAnswers.packingListReturnChange(
        basicRequestWithEmptyAnswers.copy(subscription = subscription, userAnswers = completedUserAnswers)
      )
  ...
  "warehouseListReturnChange" - {
    "should be correct if user is NOT a new importer" in {
      val subscription = RetrievedSubscription(
        ...
        "",
        UkAddress(List.empty, "", None),
        RetrievedActivity(true, true, true, importer = true, true),
        LocalDate.now(),
        List.empty,
        List.empty,
        Contact(None, None, "", ""),
    ...
    "should be correct if user is a new importer" in {
      ...
      val subscription = RetrievedSubscription(
        ...
        "",
        UkAddress(List.empty, "", None),
        RetrievedActivity(true, true, true, importer = false, true),
        LocalDate.now(),
        List.empty,
        List.empty,
        Contact(None, None, "", ""),
      ...
      val res = requiredUserAnswers.warehouseListReturnChange(
        basicRequestWithEmptyAnswers.copy(subscription = subscription, userAnswers = completedUserAnswers)
      )
  ...
  "smallProducerCheck" - {
    "should be correct if user is NOT a new small producer" in {
      val subscription = RetrievedSubscription(
        ...
        "",
        UkAddress(List.empty, "", None),
        RetrievedActivity(smallProducer = true, true, true, true, true),
        LocalDate.now(),
        List.empty,
        List.empty,
        Contact(None, None, "", ""),
    ...
    "should be correct if user is a small producer" in {
      val subscription = RetrievedSubscription(
        ...
        "",
        UkAddress(List.empty, "", None),
        RetrievedActivity(smallProducer = false, true, true, true, true),
        LocalDate.now(),
        List.empty,
        List.empty,
        Contact(None, None, "", ""),
