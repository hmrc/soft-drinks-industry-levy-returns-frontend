...
class RemoveSmallProducerConfirmControllerSpec extends SpecBase with MockitoSugar with LoggerHelper {
  ...
  val userAnswersData: JsObject = Json.obj(
    RemoveSmallProducerConfirmPage.toString -> Json
      .obj("producerName" -> producerName, "referenceNumber" -> sdilReference, "lowBand" -> litres, "highBand" -> litres)
  )
  ...
  "RemoveSmallProducerConfirm Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        contentAsString(result) mustEqual view(form, NormalMode, sdilReferenceParty, producerNameParty)(using request, messages(application)).toString
      }
    }

    "must return OK and the correct view for a GET when yes or no was already answered on previous visits to the page" in {
      val userAnswers =
        emptyUserAnswers.copy(data = userAnswersData, smallProducerList = smallProducerList).set(RemoveSmallProducerConfirmPage, false).success.value
      val application = applicationBuilder(userAnswers = Some(userAnswers)).build()
    ...
    "must redirect to the next page when valid data is submitted" in {
      val userAnswers =
        emptyUserAnswers.copy(data = userAnswersData, smallProducerList = smallProducerList).set(RemoveSmallProducerConfirmPage, true).success.value
    ...
    "must return to small producer details page when no is selected and there are two producers on the list" in {
      val userAnswers = emptyUserAnswers
        .copy(data = userAnswersData, smallProducerList = smallProducerListWithTwoProducers)
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      running(application) {
        ...
        val page = Jsoup.parse(contentAsString(result))
        contentAsString(result) mustEqual view(boundForm, NormalMode, sdilReferenceParty, producerNameParty)(using
          request,
