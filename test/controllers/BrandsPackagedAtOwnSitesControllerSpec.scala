...
import models.{NormalMode, BrandsPackagedAtOwnSites, UserAnswers}
...
class BrandsPackagedAtOwnSitesControllerSpec extends SpecBase with MockitoSugar {
  ...
  "BrandsPackagedAtOwnSites Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[BrandsPackagedAtOwnSitesView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(BrandsPackagedAtOwnSites(value1, value2)), NormalMode)(request, messages(application)).toString
      }
    }
