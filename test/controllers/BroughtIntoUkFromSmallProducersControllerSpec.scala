...
class BroughtIntoUkFromSmallProducersControllerSpec extends SpecBase with MockitoSugar with LoggerHelper {
  ...
  "BroughtIntoUkFromSmallProducers Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(BroughtIntoUkFromSmallProducersPage, true).success.value
        ...
        withCaptureOfLoggingFrom(application.injector.instanceOf[GenericLogger].logger) { events =>
          ...
          events.collectFirst {
            case event =>
              ...
              event.getMessage mustEqual ("Failed to resolve user answers while on broughtIntoUkFromSmallProducers")
      ...
      running(app) {
        withCaptureOfLoggingFrom(application.injector.instanceOf[GenericLogger].logger) { events =>
          ...
          events.collectFirst {
            case event =>
              event.getLevel.levelStr mustEqual ("ERROR")
              event.getMessage mustEqual ("Failed to set value in session repository while attempting set on broughtIntoUkFromSmallProducers")
          }.getOrElse(fail("No logging captured"))
        }
      }
    }
