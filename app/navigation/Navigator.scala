...
class Navigator @Inject() () {
  ...
  private val normalRoutes: Page => UserAnswers => Option[SdilReturn] => Option[RetrievedSubscription] => Option[Boolean] => Call = {
    case ReturnChangeRegistrationPage =>
      _ => sdilReturnOpt => subscriptionOpt => _ => returnChangeRegistrationPageNavigation(sdilReturnOpt, subscriptionOpt)
    ...
    case HowManyCreditsForLostDamagedPage       =>
      _ => sdilReturnOpt => subscriptionOpt => _ => howManyCreditsForLostDamagedPageNavigation(sdilReturnOpt, subscriptionOpt)
    case ClaimCreditsForLostDamagedPage =>
      userAnswers => sdilReturnOpt => subscriptionOpt => _ => claimCreditsForLostDamagedPageNavigation(userAnswers, sdilReturnOpt, subscriptionOpt)
    ...
    case SmallProducerDetailsPage                      =>
      userAnswers => _ => _ => smallProducerMissing => smallProducerDetailsPageNavigation(userAnswers, smallProducerMissing)
  ...
  private val checkRouteMap: Page => UserAnswers => Option[SdilReturn] => Option[RetrievedSubscription] => Call = {
    case OwnBrandsPage                   => userAnswers => _ => _ => checkOwnBrandPageNavigation(userAnswers)
    case BrandsPackagedAtOwnSitesPage    => _ => _ => _ => routes.CheckYourAnswersController.onPageLoad
    case PackagedContractPackerPage      => userAnswers => _ => _ => checkPackagedContractPackerPageNavigation(userAnswers)
    case HowManyAsAContractPackerPage    => _ => sdilReturn => subscription => nextPageBasedNewPackerStatus(sdilReturn, subscription)
    case ExemptionsForSmallProducersPage => userAnswers => _ => _ => checkExemptionForSmallProducersPageNavigation(userAnswers)
    case SmallProducerDetailsPage        =>
      userAnswers => sdilReturn => subscription => checkSmallProducerDetailsPageNavigation(userAnswers, sdilReturn, subscription)
    case AddASmallProducerPage                         => _ => _ => _ => routes.SmallProducerDetailsController.onPageLoad(CheckMode)
    case RemoveWarehouseConfirmPage                    => _ => _ => _ => routes.SecondaryWarehouseDetailsController.onPageLoad(CheckMode)
    case RemoveSmallProducerConfirmPage                => userAnswers => _ => _ => removeSmallProducerConfirmPageNavigation(userAnswers, CheckMode)
    case BroughtIntoUKPage                             => userAnswers => _ => _ => checkBroughtIntoUkPageNavigation(userAnswers)
    case HowManyBroughtIntoUkPage                      => _ => _ => _ => routes.CheckYourAnswersController.onPageLoad
    case BroughtIntoUkFromSmallProducersPage           => userAnswers => _ => _ => checkBroughtIntoUkFromSmallProducersPageNavigation(userAnswers)
    case HowManyBroughtIntoTheUKFromSmallProducersPage => _ => _ => _ => routes.CheckYourAnswersController.onPageLoad
    case ClaimCreditsForExportsPage                    => userAnswers => _ => _ => checkClaimCreditsForExportPageNavigation(userAnswers)
    case HowManyCreditsForExportPage                   => _ => _ => _ => routes.CheckYourAnswersController.onPageLoad
    case ClaimCreditsForLostDamagedPage                => userAnswers => _ => _ => checkClaimCreditsForLostDamagedPageNavigation(userAnswers)
    case HowManyCreditsForLostDamagedPage              => _ => _ => _ => routes.CheckYourAnswersController.onPageLoad
    case RemovePackagingDetailsConfirmationPage        => _ => _ => _ => routes.PackagingSiteDetailsController.onPageLoad(CheckMode)
  ...
  def nextPage(
    ...
    mode match {
      ...
      case CheckMode  => checkRouteMap(page)(userAnswers)(sdilReturn)(subscription)
  ...
  private def ownBrandPageNavigation(userAnswers: UserAnswers) =
    if userAnswers.get(page = OwnBrandsPage).contains(true) then {
  ...
  private def packagedContractPackerPageNavigation(userAnswers: UserAnswers) =
    if userAnswers.get(page = PackagedContractPackerPage).contains(true) then {
  ...
  private def exemptionForSmallProducersPageNavigation(userAnswers: UserAnswers) =
    if userAnswers.get(page = ExemptionsForSmallProducersPage).contains(true) && userAnswers.smallProducerList.isEmpty then {
      routes.AddASmallProducerController.onPageLoad(NormalMode)
    } else if userAnswers.get(page = ExemptionsForSmallProducersPage).contains(false) then {
  ...
  private def broughtIntoUkPageNavigation(userAnswers: UserAnswers) =
    if userAnswers.get(page = BroughtIntoUKPage).contains(true) then {
  ...
  private def broughtIntoUkfromSmallProducersPageNavigation(userAnswers: UserAnswers) =
    if userAnswers.get(page = BroughtIntoUkFromSmallProducersPage).contains(true) then {
  ...
  private def smallProducerDetailsPageNavigation(userAnswers: UserAnswers, smallProducerMissing: Option[Boolean]) =
    smallProducerMissing match {
      ...
      case _          =>
        if userAnswers.get(page = SmallProducerDetailsPage).contains(true) then {
          routes.AddASmallProducerController.onPageLoad(BlankMode)
        } else if userAnswers.smallProducerList.isEmpty then {
          routes.ExemptionsForSmallProducersController.onPageLoad(NormalMode)
        } else {
          routes.BroughtIntoUKController.onPageLoad(NormalMode)
        }
  ...
  private def removeSmallProducerConfirmPageNavigation(userAnswers: UserAnswers, mode: Mode) =
    if userAnswers.get(page = RemoveSmallProducerConfirmPage).contains(true) && userAnswers.smallProducerList.isEmpty then {
  ...
  private def removeWarehouseConfirmPageNavigation(userAnswers: UserAnswers): Call =
    if userAnswers.get(page = RemoveWarehouseConfirmPage).contains(true) then {
  ...
  private def claimCreditsForExportPageNavigation(userAnswers: UserAnswers) =
    if userAnswers.get(page = ClaimCreditsForExportsPage).contains(true) then {
  ...
  private def claimCreditsForLostDamagedPageNavigation(
    userAnswers:     UserAnswers,
    sdilReturnOpt:   Option[SdilReturn],
    subscriptionOpt: Option[RetrievedSubscription]
  ) =

    if userAnswers.get(page = ClaimCreditsForLostDamagedPage).contains(true) then {
  ...
  private def howManyCreditsForLostDamagedPageNavigation(sdilReturnOpt: Option[SdilReturn], subscriptionOpt: Option[RetrievedSubscription]) =
    packerImporterPageNavigation(sdilReturnOpt, subscriptionOpt)

  private def packerImporterPageNavigation(sdilReturnOpt: Option[SdilReturn], subscriptionOpt: Option[RetrievedSubscription]) =

    (sdilReturnOpt, subscriptionOpt) match {
      case (Some(sdilReturn), Some(subscription)) =>
        if UserTypeCheck.isNewImporter(sdilReturn, subscription) || UserTypeCheck.isNewPacker(sdilReturn, subscription) then {
  ...
  private def checkExemptionForSmallProducersPageNavigation(userAnswers: UserAnswers) =
    ...
    } else if userAnswers.get(page = ExemptionsForSmallProducersPage).contains(true) && userAnswers.smallProducerList.nonEmpty then {
      routes.SmallProducerDetailsController.onPageLoad(CheckMode)
    } else {
      routes.CheckYourAnswersController.onPageLoad
    }

  private def checkSmallProducerDetailsPageNavigation(
    userAnswers:  UserAnswers,
    ...
    if userAnswers.get(page = SmallProducerDetailsPage).contains(true) then {
      ...
      nextPageBasedNewPackerStatus(sdilReturn, subscription)
    }

  private def returnChangeRegistrationPageNavigation(sdilReturnOpt: Option[SdilReturn], subscriptionOpt: Option[RetrievedSubscription]) =
    (sdilReturnOpt, subscriptionOpt) match {
      case (Some(sdilReturn), Some(subscription)) =>
        if newPackerWithNoExistingPackingSites(sdilReturn, subscription) then {
  ...
  private def nextPageBasedNewPackerStatus(sdilReturn: Option[SdilReturn], subscription: Option[RetrievedSubscription]) =
    (sdilReturn, subscription) match {
      case (Some(sdilReturn), Some(subscription)) =>
        if newPackerWithNoExistingPackingSites(sdilReturn, subscription) then {
          routes.PackAtBusinessAddressController.onPageLoad(CheckMode)
        } else {
          routes.CheckYourAnswersController.onPageLoad
        }
      case _ =>
  ...
  private def newPackerWithNoExistingPackingSites(sdilReturn: SdilReturn, subscription: RetrievedSubscription) =
    UserTypeCheck.isNewPacker(sdilReturn, subscription) && subscription.productionSites.isEmpty
