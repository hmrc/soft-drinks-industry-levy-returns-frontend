    ...
    answers.get(HowManyBroughtIntoTheUKFromSmallProducersPage).map {
      ...
      val value = HtmlFormat.escape(answer.lowBand.toString).toString + "<br/>" + HtmlFormat.escape(answer.highBand.toString).toString
        ...
        SummaryListRowViewModel(
          key     = "howManyBroughtIntoTheUKFromSmallProducers.checkYourAnswersLabel",
          ...
          actions = Seq(
            ActionItemViewModel("site.change", routes.HowManyBroughtIntoTheUKFromSmallProducersController.onPageLoad(CheckMode).url)
              .withVisuallyHiddenText(messages("howManyBroughtIntoTheUKFromSmallProducers.change.hidden"))
          )
        )
    }
}
