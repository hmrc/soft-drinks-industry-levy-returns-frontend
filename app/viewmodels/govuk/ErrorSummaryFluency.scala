...
trait ErrorSummaryFluency {
  ...
  object ErrorSummaryViewModel {
    ...
    def apply(form: Form[?], errorLinkOverrides: Map[String, String] = Map.empty)(implicit messages: Messages): ErrorSummary = {

      val errors = form.errors.map { error =>
        ErrorLink(href = Some(s"#${errorLinkOverrides.getOrElse(error.key, error.key)}"), content = Text(messages(error.message, error.args*)))
      }
