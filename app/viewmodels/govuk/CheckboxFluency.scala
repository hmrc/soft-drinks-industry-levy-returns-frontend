...
trait CheckboxFluency {
  ...
  object CheckboxesViewModel extends ErrorMessageAwareness with FieldsetFluency {
    ...
    def apply(form: Form[?], name: String, items: Seq[CheckboxItem], legend: Legend)(implicit messages: Messages): Checkboxes =
      apply(form = form, name = name, items = items, fieldset = FieldsetViewModel(legend))

    def apply(form: Form[?], name: String, items: Seq[CheckboxItem], fieldset: Fieldset)(implicit messages: Messages): Checkboxes =
      Checkboxes(
