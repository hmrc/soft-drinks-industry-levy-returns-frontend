...
class CheckYourAnswersController @Inject() (
  ...
  def onPageLoad: Action[AnyContent] = (identify andThen getData andThen requireData andThen checkReturnSubmission).async { implicit request =>
  ...
  def onSubmit: Action[AnyContent] = (identify andThen getData andThen requireData andThen checkReturnSubmission).async { implicit request =>
    requiredUserAnswers.requireData(CheckYourAnswersPage) {
  ...
  private def sanitisePackingSitesInAnswers(answers: UserAnswers, subscription: RetrievedSubscription) =
    if !UserTypeCheck.isNewPacker(SdilReturn.apply(answers), subscription) then {
  ...
  private def sanitiseWarehousesInAnswers(answers: UserAnswers, subscription: RetrievedSubscription) =
    if !UserTypeCheck.isNewImporter(SdilReturn.apply(answers), subscription) then {
