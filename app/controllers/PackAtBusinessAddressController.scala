...
class PackAtBusinessAddressController @Inject() (
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData andThen checkReturnSubmission) { implicit request =>
    val formattedAddress: HtmlContent =
      AddressFormattingHelper.formatBusinessAddress(request.subscription.address, Some(request.subscription.orgName))
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData andThen checkReturnSubmission).async {
    implicit request =>
      ...
      form
        .bindFromRequest()
        .fold(
          formWithErrors => Future.successful(BadRequest(view(formWithErrors, formattedAddress, mode))),
          value =>
            for {
              ...
              onwardUrl      <- if value then {
                             ...
                             updateDatabaseWithoutRedirect(updatedAnswers, PackAtBusinessAddressPage)
