...
class RampOffController @Inject() (
  ...
  def secondaryWareHouseDetailsOffRamp(siteId: String, alfId: String, mode: Mode): Action[AnyContent] =
    (identify andThen getData andThen requireData).async { implicit request =>
      ...
      } yield Redirect(controllers.routes.SecondaryWarehouseDetailsController.onPageLoad(mode))
    }
  ...
  def packingSiteDetailsOffRamp(siteId: String, alfId: String, mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      for {
        ...
        _ <- sessionRepository.set(updatedUserAnswers)
      } yield Redirect(controllers.routes.PackagingSiteDetailsController.onPageLoad(mode))
  }
}
