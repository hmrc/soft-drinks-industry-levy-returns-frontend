...
class AuthController @Inject() (
  ...
  def signOut(): Action[AnyContent] = identify.async { implicit request =>
    sdilSessionCache.removeRecord(request.subscription.sdilRef).map(_ => Redirect(config.signOutUrl, Map("continue" -> Seq(config.exitSurveyUrl))))
  }

  def signOutNoSurvey(): Action[AnyContent] = identify.async { implicit request =>
    sdilSessionCache
      .removeRecord(request.subscription.sdilRef)
