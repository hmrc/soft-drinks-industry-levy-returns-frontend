...
class AddASmallProducerController @Inject() (
              ...
              case _ =>
                ...
                sdilConnector.checkSmallProducerStatus(value.referenceNumber, request.returnPeriod).flatMap {
                  case Some(false) =>
                    Future.successful(
                      BadRequest(
                        view(preparedForm.withError(FormError("referenceNumber", "addASmallProducer.error.referenceNumber.notASmallProducer")), mode)
                      )
                    )
                  case _ =>
                    updateDatabase(value, userAnswers)
                      .map(updatedAnswersFinal => Redirect(navigator.nextPage(AddASmallProducerPage, mode, updatedAnswersFinal)))
                }
            }
          }
        )
  ...
  def onEditPageSubmit(mode: Mode, sdilReference: String): Action[AnyContent] =
    (identify andThen getData andThen requireData).async { implicit request =>
      ...
      form
        .bindFromRequest()
        .fold(
          formWithErrors => Future.successful(BadRequest(view(formWithErrors, mode, Some(sdilReference)))),
          formData => {
            val smallProducerList = request.userAnswers.smallProducerList
            isValidSDILRef(sdilReference, formData.referenceNumber, smallProducerList, returnPeriod).flatMap {
              case Left(AlreadyExists) =>
                Future.successful(
                  BadRequest(
                    view(form.withError(FormError("referenceNumber", "addASmallProducer.error.referenceNumber.exists")), mode, Some(sdilReference))
                  )
                )
              ...
              case Right(_) =>
                updateSmallProducerList(formData, userAnswers, sdilReference)
                  .map(updatedAnswersFinal => Redirect(navigator.nextPage(AddASmallProducerPage, mode, updatedAnswersFinal)))
            }
          }
        )
  ...
  private def isValidSDILRef(
    currentSDILRef:           String,
    addASmallProducerSDILRef: String,
    smallProducerList:        Seq[SmallProducer],
    returnPeriod:             ReturnPeriod
  )(implicit hc: HeaderCarrier): Future[Either[SDILReferenceErrors, Unit]] =
    if currentSDILRef == addASmallProducerSDILRef then {
      Future.successful(Right(()))
    } else if smallProducerList.map(_.sdilRef).contains(addASmallProducerSDILRef) then {
