...
trait ControllerHelper extends FrontendBaseController with I18nSupport {
  ...
  def updateDatabaseAndRedirect(
    ...
    updatedAnswers match {
      ...
      case Success(answers) =>
        val sdilReturn: Option[SdilReturn] = if withSdilReturn then {
  ...
  def setAndRedirect(
    ...
    sessionRepository.set(updatedAnswers).flatMap {
      case Right(_) => Future.successful(Redirect(navigator.nextPage(page, mode, updatedAnswers, sdilReturn, subscription, smallProducerMissing)))
      case Left(_)  =>
  ...
  private def failure(page: Page)(implicit ec: ExecutionContext, request: Request[?]): Future[Result] = {
