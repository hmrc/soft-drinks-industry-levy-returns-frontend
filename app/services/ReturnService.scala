...
import util.ReturnsHelper.{extractTotal, listItemsWithTotal}
import util.{TotalForQuarter, UserTypeCheck}
...
class ReturnService @Inject() (sdilConnector: SoftDrinksIndustryLevyConnector, config: FrontendAppConfig) {
  ...
  def sendReturn(subscription: RetrievedSubscription, returnPeriod: ReturnPeriod, userAnswers: UserAnswers)(implicit
    hc: HeaderCarrier,
    ec: ExecutionContext
  ): Future[Unit] =
    submitReturnAndVariation(subscription, returnPeriod, userAnswers)

  private[services] def submitReturnAndVariation(subscription: RetrievedSubscription, returnPeriod: ReturnPeriod, userAnswers: UserAnswers)(implicit
    hc: HeaderCarrier,
    ec: ExecutionContext
  ): Future[Unit] = {
  ...
  private def importsLitres(userAnswers: UserAnswers) =
    (userAnswers.get(HowManyBroughtIntoUkPage).map(_.lowBand).getOrElse(0L), userAnswers.get(HowManyBroughtIntoUkPage).map(_.highBand).getOrElse(0L))
