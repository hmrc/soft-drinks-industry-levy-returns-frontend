...
class AddressLookupService @Inject() (addressLookupConnector: AddressLookupConnector, frontendAppConfig: FrontendAppConfig) extends AddressHelper {
  ...
  def addressChecker(address: AlfAddress, alfId: String): UkAddress = {
    ...
    if ukAddress.lines.isEmpty && ukAddress.postCode == "" && address.organisation.isEmpty then {
  ...
  def initJourneyAndReturnOnRampUrl(state: AddressLookupState, siteId: String = generateId, mode: Mode = NormalMode)(implicit
    hc:            HeaderCarrier,
    ec:            ExecutionContext,
    messages:      Messages,
  ...
  def createJourneyConfig(state: AddressLookupState, siteId: String, mode: Mode = NormalMode)(implicit
    ...
    JourneyConfig(
      ...
      options = JourneyOptions(
        ...
        phaseFeedbackLink = Some(frontendAppConfig.feedbackUrl(using requestHeader)),
      ...
      case PackingDetails =>
        Some(
          JourneyLabels(
            en = Some(
              LanguageLabels(
                appLevelLabels = Some(AppLevelLabels(navTitle = Some(messages("service.name")), phaseBannerHtml = None)),
