...
@Singleton
class ReturnsOrchestrator @Inject() (returnService: ReturnService, sdilSessionCache: SDILSessionCache, sessionRepository: SessionRepository) {

  def handleReturnRequest(year: Int, quarter: Int, nilReturn: Boolean)(implicit
    request: OptionalDataRequest[AnyContent],
    hc:      HeaderCarrier,
    ec:      ExecutionContext
  ): ReturnResult[Unit] = EitherT {
    ...
    val userAnswersForReturnPeriod =
      request.userAnswers.fold[Option[UserAnswers]](None) { userAnswers =>
        if userAnswers.returnPeriod == requestReturnPeriod then {
          Some(userAnswers)
        } else {
          None
        }
  ...
  def setupNewReturn(returnPeriod: ReturnPeriod, nilReturn: Boolean)(implicit
    request: OptionalDataRequest[AnyContent],
    hc:      HeaderCarrier,
    ec:      ExecutionContext
  ): ReturnResult[Unit] =
  ...
  def checkIfValidReturnPeriod(
    returnPeriod: ReturnPeriod
  )(implicit request: OptionalDataRequest[AnyContent], hc: HeaderCarrier, ec: ExecutionContext): ReturnResult[ReturnPeriod] = EitherT {

    returnService.getPendingReturns(request.subscription.utr).map { pendingReturns =>
      if pendingReturns.contains(returnPeriod) then {
        Right(returnPeriod)
      } else {
  ...
  def calculateAmounts(sdilRef: String, userAnswers: UserAnswers, returnPeriod: ReturnPeriod)(implicit
    hc: HeaderCarrier,
    ec: ExecutionContext
  ): Future[Amounts] =
  ...
  def getCalculatedAmountsForReturnSent(sdilRef: String, userAnswers: UserAnswers, returnPeriod: ReturnPeriod)(implicit
    hc: HeaderCarrier,
    ec: ExecutionContext
  ): Future[Amounts] =
